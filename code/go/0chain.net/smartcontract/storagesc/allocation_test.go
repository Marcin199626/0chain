...
func TestSelectBlobbers(t *testing.T) {
	...
	setup := func(t *testing.T, args args) (
		...
		balances.On("GetTrieNode", ALL_BLOBBERS_KEY, mock.Anything).Return(
			func(_ datastore.Key, serializable util.MPTSerializable) error {
				part := makePartitionFromBlobbers(sNodes, balances, t)
				partSer := part.(msgp.Marshaler)
				blob, err := partSer.MarshalMsg(nil)
...
func TestStorageSmartContract_newAllocationRequest(t *testing.T) {
	t.Skip("broken cause msgp implementation")
	...
	assert.EqualValues(t, sb, aresp.Blobbers)
	// blobbers saved in all blobbers list
	allBlobbers, err = getBlobberList(balances)
	require.NoError(t, err)
...
func addBlobbers(sc *StorageSmartContract, blobbers []*StorageNode, balances chainState.StateContextI, t *testing.T) {
	for _, blobber := range blobbers {
		var (
			txn = transaction.Transaction{
				ClientID:  blobber.ID,
				PublicKey: blobber.PublicKey,
			}

			input = blobber.Encode()
		)

		_, err := sc.addBlobber(&txn, input, balances)
		require.NoError(t, err)
	}
}
...
func Test_preferred_blobbers(t *testing.T) {
	...
	t.Run("unhealthy preferred blobbers", func(t *testing.T) {
		...
		var updateBlobber = func(t *testing.T, b *StorageNode) {
			t.Helper()
			//var all, err = getBlobberList(balances)
			//require.NoError(t, err)
			//all.Nodes.update(b)
